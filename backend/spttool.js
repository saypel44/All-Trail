...
/* ═══════════════════════════════════════
   SURVEY & AI TIPS LOGIC
   ═══════════════════════════════════════ */
async function submitCheckIn() {
    const score = sleepScore();
    const fb = buildFeedback(); // This generates the "Tips"
    ...
    try {
        // Save to your MySQL backend
        await _apiRequest('/api/checkins', 'POST', payload);
        
        // Update UI to show the "Results" screen
...
function renderAIFeedback(container, fb) {
    container.innerHTML = `
        <div class="ai-feedback-card" style="padding: 20px; background: var(--bg-card); border-radius: 12px; border: 1px solid var(--accent);">
            <h3 style="color: var(--accent);">✨ Your Personal Recommendations</h3>
            <p><strong>Focus Area:</strong> ${fb.areaOfImprovement}</p>
            <ul style="text-align: left; margin: 15px 0;">
                ${fb.actions.map(a => `<li style="margin-bottom: 8px;">🎯 ${a}</li>`).join('')}
            </ul>
            <button onclick="location.reload()" class="restart-btn" style="width: 100%; padding: 12px; background: var(--accent); color: white; border: none; border-radius: 8px; cursor: pointer;">Start New Check-in</button>
        </div>
    `;
}

/* ═══════════════════════════════════════
   FIX: TAB NAVIGATION (Tracker, History, Trends)
   ═══════════════════════════════════════ */
function showTab(tabName) {
    ...
    // 5. Refresh data for specific tabs
...
function calculateDuration(start, end, unit) {
    ...
    if (diffMins < 0) diffMins += 1440; // Handles wrap-around midnight
