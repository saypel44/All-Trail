...
const stmts = {
  ...
  insertAlarm:   db.prepare(`
    INSERT INTO alarms (username, from_time, to_time, category, sound, date, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `),
...
const server = http.createServer(async (req, res) => {
  ...
  // ── GET /api/logs ────────────────────────────────────
  // Returns all habit logs for the signed-in user
  if (req.method === 'GET' && url === '/api/logs') {
    ...
    // Convert DB column names back to camelCase for the app
  ...
  // ── DELETE /api/logs/:id ─────────────────────────────
  // Delete a single log entry (only if it belongs to this user)
  ...
  // ── GET /api/alarms ──────────────────────────────────
  // Returns all alarms for the signed-in user
