...
function doLogin() {
  ...
  if (remember) {
    localStorage.setItem('qt_remember', JSON.stringify({ user, pass }));
  } else {
    localStorage.removeItem('qt_remember');
  }
...
function doLogout() {
  ...
  clearAuthMsg();
  // Re-fill remembered credentials if present
  const remembered = _loadRemembered();
  document.getElementById('li-user').value = remembered.user;
  document.getElementById('li-pass').value = remembered.pass;
  document.getElementById('li-remember').checked = !!remembered.user;
  window.scrollTo(0, 0);
}
function _loadRemembered() {
  try {
    const r = JSON.parse(localStorage.getItem('qt_remember') || 'null');
    return r || { user: '', pass: '' };
  } catch(e) { return { user: '', pass: '' }; }
}

// Wire up keyboard shortcuts, pre-fill credentials, and auto-login
